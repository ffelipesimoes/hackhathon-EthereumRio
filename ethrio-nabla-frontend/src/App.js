...
const ShowUniswapObserveValues = (props) => {
  const { data, runContractFunction, isFetching} =
    useWeb3Contract({
      abi: usdcEthPoolAbi,
  ...
  return (
    <div>
      <Button type="primary" onClick={() => runContractFunction()} disabled={isFetching}>
        Fetch data
      </Button>
...
const App = () => {
    ...
    body=
      <>
        <div style={{textAlign:'center', minHeight: '100vh'}}>
          <h1>Usuario não autenteicado</h1>
        </div>
      </>
  }
  else{
  ...
  <>   
  {body}
      <ModalComponent text="Qual sua proposta?" isModalVisible={isSubmmitModalVisible} handleCancel={handleSubmmitCancel}/>
      <Modal title="Participe de uma votação" visible={isVoteModalVisible} onCancel={handleVoteCancel} footer={null}>
        <a>Id da proposta</a>
        <Input placeholder=""/>
        <p/>
        <Row>
